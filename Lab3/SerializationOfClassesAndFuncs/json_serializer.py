...
class JsonSerializer(BaseSerializer):
    ...
    def dumps(self, obj) -> str:
        ...
        return self._dumps_from_dict(obj)

    def _dumps_from_dict(self, obj) -> str:
        obj_type = type(obj)
        ...
        if obj_type is str:
            return f'"{self.__mask_quotes(obj)}"'

        if obj_type is nonetype:
            return self.NULL_LITERAL

        if obj_type is list:
            return '[' + ", ".join([self._dumps_from_dict(item) for item in obj]) + ']'

        if obj_type is dict:
            return '{' + ", ".join([f"{self._dumps_from_dict(key)}: {self._dumps_from_dict(value)}"
    ...
    def __loads_to_dict(self, string: str):
        ...
        if string.startswith('{') and string.endswith('}'):
            ...
            # matches will store key-value pairs in one row. Elements with
